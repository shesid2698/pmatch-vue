<template>
  <div class="menu-container hidden lg:block">
    ...
    <NuxtLink v-for="page of curPage" class="decoration-none" :to="page.link">
      <div
        class="p-x-15px p-y-7px text-black font-bold cursor-pointer hover:opacity-50 hover:underline whitespace-nowrap"
        :class="page.show">
        {{ page.title }}
      </div>
    </NuxtLink>
...
onMounted(async () => {
  ...
  switch (route.path) {
    ...
    case "/member/center/recommend":
      curPage.value[8].show = "showTitle";
      break;
  }
});
</script>
