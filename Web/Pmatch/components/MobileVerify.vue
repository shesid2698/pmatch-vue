<template>
  ...
  <div class="ccontainer">
    ...
    <div>
      ...
      <div>
        <input type="text" v-model="mobile" @input="validMobilePattern" :disabled="countdown !== 120 && countdown !== 0"
      ...
      <div>
        <button :disabled="openSendBtn()" @click="SendCode"
          class="w-50% bg-[#198754] h-38px text-white outline-none border-none rounded-1 hover:bg-[#157347] cursor-pointer disabled:bg-gray disabled:cursor-default">發送驗證碼</button><span
          v-show="countdown !== 120 && countdown !== 0">{{ countdown }} 秒後重置...</span>
      </div>
    </div>
    <div class="mt-15px">
      <div class="mb-5px text-[#484646] font-400 text-15px">
      ...
      <div>
        <input type="text" v-model="ansCode"
          class="box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200" />
      </div>
      <div class="mt-15px">
...
const SendCode = async () => {
  try {
    ...
    if (response.data.Status.Code === 0) {
      await openAlertModal(" ", '驗證碼已發送，請至手機收取驗證碼!');
      startCountdown();
