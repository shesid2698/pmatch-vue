    ...
    <div class="flex-1 md:pl-20px">
      ...
      <div>
        ...
        <el-table :data="tableData" :default-sort="{ prop: 'Createtime', order: 'descending' }" style="width: 100%"
          ...
          <el-table-column prop="GameplatformName" label="營運平台" />
          <el-table-column prop="NickName" label="媒合商" />
          <el-table-column prop="Patch" label="數量" />
          <el-table-column prop="GamecurrencyName" label="單位" />
          <el-table-column prop="Status" label="狀態" />
        </el-table>
        <div class="flex justify-between">
          <div>頁次:{{ curPage }}/{{ totalPage }}</div>
          <div>
            <button v-if="curPage !== 1" @click="PrevPage"
              class="bg-primary border-none text-[15px] rounded-[5px] bg-#1A6DB4 hover:bg-#0b5ed7 p-x-10px p-y-5px text-white cursor-pointer">
              上一頁
            </button>
            <button @click="NextPage" v-if="curPage !== totalPage"
              class="bg-primary border-none text-[15px] rounded-[5px] bg-#1A6DB4 hover:bg-#0b5ed7 p-x-10px p-y-5px text-white cursor-pointer">
              下一頁
            </button>
          </div>
