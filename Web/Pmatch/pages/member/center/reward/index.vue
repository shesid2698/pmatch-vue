<template>
  ...
  <div v-else class="ccontainer pt-60px ps-5 pe-5 w-90% lg:w-70%">
    ...
    <div class="flex-1 md:pl-20px">
      ...
      <div v-if="pageName === ''">
        <div>
          <button class="non-button" @click="SetInnerPage(1)" :class="{ 'active-button': innerPage == 1 }">可使用</button>
          <button class="non-button" @click="SetInnerPage(2)" :class="{ 'active-button': innerPage == 2 }">處理中</button>
          <button class="non-button" @click="SetInnerPage(3)" :class="{ 'active-button': innerPage == 3 }">已使用</button>
          <button class="non-button" @click="SetInnerPage(4)" :class="{ 'active-button': innerPage == 4 }">已過期</button>
        </div>
        <div class="ticket-list max-h-624px overflow-auto" @scroll="GetMoreData">
          <div v-for="item in ticketList" :key="item.ActivityId">
            <div class="flex ticket-shadow mb-2">
              <div class="relative w-fit mr-[-2px] h-fit">
                ...
                <div class="absolute w-67% bottom-10px right-0 text-end pr-10px text-10px md:text-16px text-black">{{
              ...
              <div class="relative w-fit h-fit cursor-pointer" @click="GetReward($event, item)">
                <div v-if="item.RewardType === 1"
                  class="absolute w-80px bg-[#3dadff] p-2 rounded-1 text-white text-center top-50% left-50% z-9 tips-shadow">
                  複製成功</div>
        ...
        <div class="max-h-296px md:w-712px w-352px overflow-auto rule-container">
          ...
          <br>
          <br>
          1. 遊戲內序號代碼用來透過遊戲用戶端或網路瀏覽器兌換遊戲內貨幣，兌換前請先登入帳號，並確認已於遊戲內創立遊戲角色，否則將無法兌換。
          <br>
          <br>
          2. 請注意兌換序號的使用條件與兌換期限，超出限制則無法兌換，請盡快登入遊戲領取。
          <br>
          <br>
          3. 同一兌換序號僅可使用一次，同一角色不可重複兌換使用相同類型的兌換序號。
          <br>
          <br>
          4. 兌換序號兌換後無法撤回，請仔細確認登入帳號是否為需要領取的帳號
          <br>
          <br>
          5. 若兌換時遇到問題，請聯繫遊戲公司客服。
          <br>
          <br>
          二、【媒合商遊戲虛寶】兌換規則
          <br>
          <br>
          1. 玩家下單兌換時，需填寫正確對應的遊戲角色名稱，以及兌換序號才可進行兌換獎勵。
          <br>
          <br>
          2. 兌換序號成功兌換後將無法撤回，請注意。
          <br>
          <br>
          3. 成功送出兌換單據後，請耐心等待媒合商處理發送獎勵，發送完成後，領獎券會顯示「已使用」。
          <br>
          <br>
          4. 請注意領獎券的使用條件與兌換期限，超出限制則無法兌換。
          <br>
          <br>
          三、【實體贈品】兌換規則
          <br>
          <br>
          1. 【贈品兌換】獎項寄送地址以中獎人會員中心之個人資料(姓名、聯絡電話、聯絡地址)為準，如無法配合提供申報相關文件，或因資料不齊全，以致無法寄送獎項，將視同放棄領獎資格。
          <br>
          <br>
          2. 寄送作業僅限臺澎金馬地區，海外人士請提供居住於臺澎金馬地區親友的地址收取。
          <br>
          <br>
          3. 【贈品寄送】主辦單位將所有得獎者資料彙整無誤後，預計於7~10天，以掛號方式寄送獎項(若有異動將另行通知)。
          <br>
          <br>
          4. 【贈品說明】活動網站之贈品圖片為示意圖，僅供參考，贈品則以實品為主，恕不指定顏色及款式、轉讓或要求折換現金或其它商品。
          <br>
          <br>
          5. 主辦單位保留修改活動及獎品等細節之權利。領獎後獎品之使用與維護，主辦單位概不負責。 贈品運送過程中，因非可歸責於主辦單位之事由造成損壞、延遲、錯遞或遺失，主辦單位概不負責。
          <br>
          <br>
          6.申報各類所得，依中華民國稅法規定，獎項金額若超過新台幣1,000元，獎項所得將列入個人年度綜合所得稅申報，故得獎人需提供身份證影本且依規定填寫並繳交相關收據方可領獎。
          <br>
          <br>
          7.
          依據中華民國稅法規定，中獎金額為1,000元以上者，將納入中獎者個人年度綜合所得計算。中獎獎項價值超過20,000元者(不含20,000元)，需依法扣繳10%機會中獎所得稅，待中獎者付清稅金後方可領獎。若未能依法繳納應繳稅額，即視為喪失中獎資格。
          <br>
          <br>
          8.
          依中華民國稅法規定，中獎者若非中華民國國境內設籍之國人（即依法於課稅年度內，在中華民國境內居住未達183天之本國人含大陸人士及外國人），不論中獎者所得獎項之價值，需預先扣繳20%機會中獎稅金，待中獎者付清稅金後方可領獎。若未能依法繳納應繳稅額，即視為喪失中獎資格。
          <br>
          <br>
          9. 如依其他法令規定另須代扣繳其他稅費者，亦於扣除相關金額後，始為實際發放金額。若未能依法繳納應繳稅額，即視為喪失中獎資格。
          <br>
          <br>
          10. 參加本活動者參加本活動而須支付任何稅捐皆為參加本活動者之義務，概與主辦單位無關。前述稅捐法規如有更新或變動者，依修正後之規定辦理。
          <br>
          <br>
          ...
          <br>
          <br>
          13. 如中獎人所提供之資料有缺漏或錯誤需要補件時，應於通知補件次日起7日內辦理補件事宜。未於期限內修正者，恕無法受理申請。
          <br>
          <br>
          14.本活動未盡事宜，係依中華民國相關法令補充之。
        </div>
      </div>
      <!-- 領獎表單 -->
      <div v-else-if="pageName === 'form'" class="w-370px mx-auto">
        <form @submit="SendRewardForm">
          <div class="mt-20px h-42px flex justify-between relative" id="progress">
            <div
              class="relative h-100% sticky w-42px  border border-[#e93470] border-solid border-[1px] rounded-full text-center content-center font-bold bg-[#e93470] text-white">
              1
              <div class="absolute text-black w-fit whitespace-nowrap top-120% left-50% transform-translate-x-[-50%]">
                填寫領獎資料</div>
            </div>
            <div
              class="relative  h-100% sticky w-42px border border-[#e93470] border-solid border-[1px] rounded-full text-center content-center text-[#e93470] font-bold"
              :class="{ '!bg-[#e93470] !text-white': RewardDetail.ProcessStatus.CurrentStep >= 2, 'bg-white': RewardDetail.ProcessStatus.CurrentStep < 2 }">
              2
              <div
                class="absolute text-black w-fit whitespace-nowrap top-120% left-50% transform-translate-x-[-50%] font-500">
                審核中</div>
            </div>
            <div
              class="relative h-100% sticky w-42px border border-[#e93470] border-solid border-[1px] rounded-full text-center content-center text-[#e93470] font-bold"
          ...
          <div class="mt-55px">
            ...
            <div class="relative">
              <input type="text" v-model="RewardItem.ActivityName" readonly
          ...
          <div class="mt-15px">
            ...
            <div class="relative">
              <input type="text" :value="RewardItem.RewardName" readonly
                class="password box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200" />
            </div>
          </div>
          <div class="mt-15px">
          ...
          <div class="mt-15px">
            ...
            <div class="relative">
              <input type="text" required
                ...
                class="password box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200"
                pattern="[A-Za-z][12]\d{8}" title="請輸入有效的台灣身分證字號" />
            </div>
          </div>
          <div class="mt-15px">
          ...
          <div class="mt-15px">
            ...
            <div class="flex">
              <div class="w-49%">
                <select v-model="selectedCity2" required
                  :disabled="RewardDetail.ProcessStatus.CurrentStep !== 1 && ticketList.find(x => x.ActivityId == RewardRequest.ActivityId).ReviewStatus !== 3"
                  class="box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200">
                  <option value="">請選擇</option>
                  <option v-for="(item, index) in cities2" :key="index" :value="index">
                    {{ index }}
                  </option>
                </select>
              </div>
              <div class="w-2%"></div>
              <div class="w-49%">
                <select v-model="selectedRegion2" required
                  :disabled="RewardDetail.ProcessStatus.CurrentStep !== 1 && ticketList.find(x => x.ActivityId == RewardRequest.ActivityId).ReviewStatus !== 3"
                  class="box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200">
                  <option value="">請選擇</option>
          ...
          <div class="mt-15px">
            ...
            <input type="text" required
              ...
              class="password box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200"
              pattern="\d{10}" title="請輸入10碼數字" />
          </div>
          <div class="mt-15px">
            <div class="mb-5px">電子信箱</div>
            <input type="text" required v-model="RewardRequest.Email"
              :disabled="RewardDetail.ProcessStatus.CurrentStep !== 1 && ticketList.find(x => x.ActivityId == RewardRequest.ActivityId).ReviewStatus !== 3"
              class="password box-border p-y-1.5 p-x-3 text-base w-100% outline-none rounded-1 border-solid border-1 border-[#ced4da] focus:outline-5 focus:outline-[#c2d9fe] focus:outline-offset-0 focus:border-[#A1C0E3] transition duration-200"
                ...
                <div class="relative w-fit h-fit">
                  <div @click="cancelImage(true)"
                    ...
                    class="absolute w-fit h-fit right-20px top-[-10px] cursor-pointer"><img src="/images/remove-btn.png"
                      width="20" alt=""></div>
                  <img class="w-85% m-auto" :src="`/images/${RewardDetail.RewardInfo.IdCardInfo.FrontImage}`" alt="">
                </div>
              </div>
              <div v-else>
                <div v-if="croppedImage" class="relative w-fit h-fit">
                  <div @click="cancelCrop(true)" class="absolute w-fit h-fit right-20px top-[-10px] cursor-pointer"><img
                      src="/images/remove-btn.png" width="20" alt=""></div>
                  <img class="w-85% m-auto" :src="croppedImage" alt="">
                </div>
                <button v-else type="button" class="camera-btn"
                  :disabled="RewardDetail.ProcessStatus.CurrentStep !== 1 && ticketList.find(x => x.ActivityId == RewardRequest.ActivityId).ReviewStatus !== 3"
                  @click="OpenDialog(true)">
                  <img src="/images/camera.svg" alt=""><br>拍照或上傳照片</button>
              </div>
              ...
              <div v-if="RewardDetail.RewardInfo.IdCardInfo.BackImage !== ''">
                <div class="relative w-fit h-fit">
                  <div @click="cancelImage(false)"
                    ...
                    class="absolute w-fit h-fit right-20px top-[-10px] cursor-pointer"><img src="/images/remove-btn.png"
                      width="20" alt=""></div>
                  <img class="w-85% m-auto" :src="`/images/${RewardDetail.RewardInfo.IdCardInfo.BackImage}`" alt="">
                </div>
              </div>
              <div v-else>
          ...
          <div class="text-15px text-[#f72585] cursor-pointer" @click="OpenDialog(true, false)"><input type="radio"
              value="yes" v-model="agreeIdRule" class="pointer-events-none">本人已詳閱並同意 貴公司「蒐集個人資料告知事項」</div>
          <div class="text-15px text-[#f72585] cursor-pointer" @click="OpenDialog(true, true)"><input type="radio"
              value="yes" v-model="agreeRewardRule" class="pointer-events-none">本人已詳閱並同意 貴公司「領獎規則」</div>
          <div class="text-center mt-20px">
            <button class="colorful-btn" @click="ReturnList" type="button">回上一頁</button>
          ...
          <el-dialog v-model="dialogVisible" width="500" align-center :show-close="false" :close-on-click-modal="false" append-to-body>
            <div v-if="!image">
              <div v-if="IsOpenRewardRule === false && IsOpenIdRule === false">
                <div
                  class="relative w-100% h-40px content-center text-center text-18px font-700 border-0 border-b-[2px] border-solid border-[#dee2e6]">
                  <span class="cursor-pointer flex  absolute right-10px top-50% transform-translate-y-[-50%]"
                    @click="dialogVisible = false">
                    <img width="20" src="/images/close.png" alt="">
                  </span>
                  證件上傳須知
                </div>
                <div class="p-3">
                  ...
                  <div class="id-sample-container">
                    <div class="id-sample-inner-container">
                      <img class="w-90%" src="/images/id-sample.png" alt="">
                    </div>
                  </div>
                </div>
                <div
                  class="w-100% flex justify-between items-center h-85px border-0 border-t-2px border-solid border-[#dee2e6]">
                  <div class="ml-70px">
                    <button @click="StartCam" type="button"
                      class="rounded-[20px] border-1px border-solid border-[#4361ee] bg-[#3dadff] w-150px h-50px text-white text-20px flex justify-center items-center cursor-pointer"><img
                        src="/images/camera-btn.png" class="mr-5px" width="30" alt="">拍照</button>
                  </div>
                  <div class="mr-70px"><label for="uploadImg"
                      class="rounded-[20px] border-1px border-solid border-[#4361ee] bg-[#3dadff] w-150px h-50px text-white text-20px flex justify-center items-center cursor-pointer"><img
                        src="/images/upload-btn.png" class="mr-5px" width="30" alt="">上傳</label>
                    <input type="file" id="uploadImg" ref="fileInput" @change="handleFileUpload" accept="image/*"
                      class="hidden" />
                  </div>
                </div>
              </div>
              ...
              <div v-else-if="IsOpenIdRule === true">
                ...
                <div class="px-5 pb-2">
                  ...
                  <br>
                  <br>
                  一、本公司因辦理抽獎活動而獲取您的姓名、國民身分證統一編號、戶籍地址、通訊地址；蒐集之個人資料類別為：「Ｃ○○一 辨識個人者」、「Ｃ○○三 政府資料中之辨識者」。
                  <br>
                  <br>
            ...
            <div v-else>
              <div class="flex justify-center px-5 py-2">
                <button @click="Rotate('left')" type="button"
                  class="bg-[#3dadff] rounded-3px text-12px text-white w-80px h-fit border-0 flex justify-center items-center py-1 cursor-pointer mr-10px"><img
                    src="/images/rotate-left.png" width="14" alt="">向左旋轉</button>
                <button @click="Rotate('right')" type="button"
                  class="bg-[#3dadff] rounded-3px text-12px text-white w-80px h-fit border-0 flex justify-center items-center py-1 cursor-pointer"><img
      ...
      <div v-else>
        <div class="md:w-460px w-100% m-auto">
          <form @submit="CreateAcc">
            ...
            <input type="text" name="GamePlatformName" hidden :value="RewardItem.GamePlatformName">
            <div><span class="text-[#E93470] mr-10px">兌換序號</span><span>{{ RewardItem.RedeemCode }}</span>
              <input type="text" name="RedeemCode" hidden :value="RewardItem.RedeemCode">
            </div>
            <div class="mt-2">
              <input type="text" placeholder="遊戲暱稱(必填)" required name="MemberCharacterName" class="colorful-input">
            </div>
            <div class="my-3"><span><input type="radio" class="mr-2" checked></span><span
                class="text-[#E93470]">兌換遊戲幣</span></div>
            <div><input type="text" readonly :value="RewardItem.Patch" name="Value" class="colorful-input"></div>
            <div class="mb-3">
            ...
            <div class="w-fit m-auto mb-3" @click='dialogVisible2 = true'><input type="radio" id="pmatchOrder"
                name="pmatchOrder" class="mr-2" title="請詳閱領獎規則" required><label for="pmatchOrder"
                class="text-[#E93470]">本人已詳閱並同意貴公司「領獎規則」</label></div>
            <div class="flex justify-center m-0"><button type="button" class="colorful-btn"
        ...
        <ElDialog v-model="dialogVisible2" :show-close="false" :close-on-click-modal="false" append-to-body>
          ...
          <div class="dialogBody">
            ...
            <div class="flex justify-end mt-0">
              <ElButton class="colorful-btn" type="primary" @click="dialogVisible2 = false">
                同意
              </ElButton>
            </div>
          </div>
