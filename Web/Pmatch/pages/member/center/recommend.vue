...
item, index
                ...
                <div class="w-50% text-16px text-black font-500 p-2 border-r-1px border-r-solid border-[#ced2db]">
                  {{ item.PlatformName }}
                </div>
                <div class="flex-1 p-2 text-16px text-black font-500 text-end">
                  {{ item.Value }}
                </div>
              </div>
            </el-scrollbar>
          ...
          <div class="flex">
            ...
            <div class="flex-1">
              <div class="flex justify-between items-center mb-5">
                ...
                <div v-show="timeValue !== 0"
                  class="flex items-center w-80px h-30px relative border-1px border-solid border-#ced2db">
                  <div @click="CannotSettingMemberPercent"
                  ...
                  <div class="text-black absolute right-5px top-50% transform-translate-y-[-50%]">
                    %
                  </div>
                </div>
              </div>
              <div class="flex justify-between items-center">
                ...
                <div class="w-80px bg-gray-100 h-30px relative border-1px border-solid border-#ced2db">
                  <input type="number" disabled v-model="secondPercent"
                  ...
                  <div class="text-black absolute right-5px top-43% transform-translate-y-[-50%]">
                    %
                  </div>
                </div>
              </div>
      ...
      <el-table :data="tableData" :default-sort="{ prop: 'Createtime', order: 'descending' }" style="width: 100%"
        :header-cell-style="{
          color: 'white',
          background: '#3CAADC',
          fontSize: '14px',
        ...
        <el-table-column prop="Account" label="被推薦人帳號" />
        <el-table-column prop="CreateTime" label="綁定時間">
          <template #default="scope">
            {{ formatDate(scope.row.CreateTime) }}
          </template>
        </el-table-column>
        <el-table-column prop="RewardPatch" label="已回饋遊戲幣">
          <template #default="scope">
            <div v-html="formatRewardPatch(scope.row.RewardPatch)"></div>
          </template>
        </el-table-column>
        <el-table-column prop="LastRewardTime" label="最後回饋日">
          <template #default="scope">
