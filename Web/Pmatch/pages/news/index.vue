  ...
  <div class="mt-7rem max-w-1110px m-auto lg-ps-0 ps-3 lg-pe-0 pe-3">
          ...
          <div class="flex w-100% relative">
            <div class="w-100% relative">
              <input v-model="searchQuery"
                class="storeName w-100% h-36px p-0 border-none bg-#eee font-size-1rem rounded-10px" type="text"
                placeholder="搜尋關鍵字..." />
            </div>
            <div class="absolute">
              <button class="h-38px searchBtn">
                <svg class="w-25px" version="1.1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512"
                  enable-background="new 0 0 512 512" xml:space="preserve" fill="#6c757d">
                  <path
    ...
    <div>
      <div class="flex flex-wrap">
        <NuxtLink v-for="(item, index) in filteredHelpItems" :key="item.Id"
          :class="index % 2 === 0 ? 'bg-#FF83AD' : 'bg-#7DC4FF'" class="helpBtn decoration-none color-#fff"
          :to="`/news/${item.Id}`">
